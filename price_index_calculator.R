...
laspeyres_index<-function(base_year){
    ...
    # Reading the valid raw data

    data<-read.csv("Valid_raw_data.csv",fill = TRUE,header = TRUE)

    # Finding the lastest month and year
    ...
    for(year in as.numeric(base_year:latest_entry$YEAR)){
        ...
        for(month in as.numeric(1:last_month)){
            ...
            # Creating the grid layout for log unit values
...
paasche_index<-function(base_year){
    ...
    # Creating the grid layout for value
    ...
    na.action=NULL)
     ...
     # Aggregating the data and taking the sum of value for each period for each HS6 code
    ...
    # Calculating the ratio value grid and applying the carry forward function
